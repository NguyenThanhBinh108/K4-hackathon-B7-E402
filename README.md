...
> **Hướng B — Trợ lý Học viên (Discord)** · Discord Knowledge Assistant · Zone E

### Thành Viên & Phân Công

| Tên | Role | Nhiệm vụ cụ thể |
|---|---|---|
| **Hải Đăng** | **Role 3 · Demo chính** | Toàn bộ codebase (backend + frontend) · `spec.md` §1-§9 · golden set · eval · validation · demo |
...
*(Codebase và spec do Hải Đăng thực hiện solo cho phần Role 3 — demo chính của nhóm; Role 1 và Role 2 là các mảng nghiên cứu riêng của các thành viên khác, không thuộc phạm vi `codebase/`.)*
...
### Cấu Trúc Riêng Của Nhóm (thêm vào cấu trúc chuẩn ở trên)

```
spec.md                          ← AI Spec §1-§9 (deadline 23:59 N1)
codebase/
├── HUONG_DAN_CHAY.md            ← Hướng dẫn chạy chi tiết (cấu hình, rate-limit, Discord bot)
├── backend/                     ← FastAPI (services/gemini_service.py = AI call thật)
├── frontend/                    ← Web UI demo chính
└── discord_bot/                 ← Bot Discord thật (thin client gọi backend) — mở rộng ngoài lát cắt demo chính
eval/                            ← golden set + log AI call thật + kết quả các lượt chạy
validation/                      ← feedback log + survey log + changelog
reflection/                      ← reflection cá nhân
```

### Chạy Prototype

Xem chi tiết đầy đủ (cấu hình `.env`, xử lý rate-limit, chạy Discord bot): [codebase/HUONG_DAN_CHAY.md](codebase/HUONG_DAN_CHAY.md)

```bash
cd codebase/backend
pip install -r requirements.txt
copy .env.example .env   # điền GEMINI_API_KEY (và GROQ_API_KEY để có fallback)
python main.py
# → http://localhost:8000
```
...
|---|---|---|
| PDF summarization (Gemini Flash, có retry/fallback) | **REAL** | `eval/ai_call_log.jsonl` |
| Q&A với KB context (Gemini Flash → Groq fallback) | **REAL** | `eval/ai_call_log.jsonl` |
